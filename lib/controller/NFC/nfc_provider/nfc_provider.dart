...
//           } else if (nfcOperation == NFCOperation.write) {
//             _writeToTag(nfcTag: nfcTag, dataType: dataType, url: url);
//             _message = "DONE";
//           }
...
//         {
//           return NdefMessage([
//             NdefRecord.createUri(Uri.parse(url.toString())),
//           ]);
//         }
//       case 'MAIL':
//         {
...
class NFCNotifier extends ChangeNotifier {
  ...
  // The read functionality is commented out as per your request
  /*
  Future<void> _readFromTag({required NfcTag tag}) async {
    Map<String, dynamic> nfcData = {
      'nfca': tag.data['nfca'],
  ...
  */
  ...
  Future<void> _writeToTag(
      ...
      required String dataType,
      String url = ""}) async {
    NdefMessage message = _createNdefMessage(dataType: dataType, url: url);
    await Ndef.from(nfcTag)?.write(message);
  }

  NdefMessage _createNdefMessage(
      {required String dataType, required String url}) {
    switch (dataType) {
      case 'URL':
        {
          return NdefMessage([
